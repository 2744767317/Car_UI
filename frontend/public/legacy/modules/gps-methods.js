...
async syncStartPointFromBackendGPS(showError = true) {
        ...
        try {
            const carApi = (window.CARServices && window.CARServices.carApi) || window.__CAR_SERVICES__?.carApi || null;
            if (!carApi) {
                throw new Error('接口服务未加载')
            }

            const data = await carApi.getGpsCurrent();
...
async submitRouteToBackend() {
        ...
        try {
            ...
            await carApi.submitRoute(payload);
        } catch (error) {
