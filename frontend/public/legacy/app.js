...
class PointCloudVisualizer {
    constructor() {
        ...
        this.selectedCoordinate = { x: 0, y: 0, z: 0 };
        ...
        if (typeof window !== 'undefined') {
            window.__CAR_LEGACY_APP__ = this;
            window.__CAR_LEGACY_API__ = {
                getSnapshot: () => this.getRuntimeSnapshot(),
                buildSnapshot: () => this.buildSceneSnapshot(),
                updateStartPointFromConfiguredGPS: (showError = true) => this.updateStartPointFromConfiguredGPS(showError),
                calibrateGpsAtCurrentCoordinate: () => this.calibrateGpsAtCurrentCoordinate(),
                toggleGpsTracking: () => this.toggleGpsTracking(),
                submitRouteToBackend: () => this.submitRouteToBackend(),
                setDrivingMode: (mode) => this.setDrivingMode(mode),
                applyStartPoint: (point) => this.applyStartPoint(point)
            };
        }
        this.init();
    }
